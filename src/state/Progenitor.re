  ...
  let workout = (w: workout) =>
    Json.Encode.(
      object_([
        ...
        ("started", Js.Json.boolean(w.started)),
        ("completed", Js.Json.boolean(w.completed)),
        ("setsCompleted", setsCompleted(w.setsCompleted)),
        ("timeUsedPerSet", timeUsedPerSet(w.timeUsedPerSet)),
      ])
    );
  let history = Json.Encode.array(workout);
  let state = s =>
    Json.Encode.(
      object_([
...
module Decode = {
  let timeUsedPerSet = json : timeUsedPerSet =>
    Json.Decode.{
      pushUps:
        json |> field("pushUps", Json.Decode.array(Json.Decode.float)),
