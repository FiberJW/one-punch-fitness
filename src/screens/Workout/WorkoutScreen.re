...
let baseMake = (~navigation, ~state as reductiveState: Progenitor.state, ~dispatch, _children) => {
  ...baseComponent,
  initialState: () => {
  ...
  willReceiveProps: (self) => {...self.state, currentExercise: currentExercise(reductiveState)},
  render: (self) => {
                        ...
                        (
                          ReasonReact.stringToElement(
                            ...
                            ++ string_of_int(
                                 switch self.state.currentExercise {
                                 | PushUps =>
                                   reductiveState.currentWorkout.setsCompleted.pushUps + 1
                                 | SitUps => reductiveState.currentWorkout.setsCompleted.sitUps + 1
                                 | Squats => reductiveState.currentWorkout.setsCompleted.squats + 1
                                 | _ => 1
                                 }
                               )
                            ++ " of "
                            ++ string_of_int(
                                 switch self.state.currentExercise {
                                 | PushUps =>
                                   Routines.variations[reductiveState.currentWorkout.level].pushUps.
                                     sets
                                 | SitUps =>
                                   Routines.variations[reductiveState.currentWorkout.level].sitUps.
                                     sets
                                 | Squats =>
                                   Routines.variations[reductiveState.currentWorkout.level].squats.
                                     sets
                                 | _ => 1
                                 }
                               )
                      ...
                      <Styled.SetReps>
                        (
                          ReasonReact.stringToElement(
                            self.state.currentExercise === Running ?
                              string_of_int(
                                Routines.variations[reductiveState.currentWorkout.level].run.
                                  distance
                              )
                              ++ Routines.variations[reductiveState.currentWorkout.level].run.units :
                              string_of_int(
                                Routines.variations[reductiveState.currentWorkout.level].sitUps.
                                  reps
                              )
